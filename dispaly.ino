...
void sideMenu()
{
    ...
    {
    u8g2.drawRBox(0,12,20,21,3);  //音乐盒子u8g2.drawRBox(盒子左边距,盒子上边距,盒子宽度,盒子高度,盒子菱角弧形);
    u8g2.setDrawColor(0);//0 盒子有颜色填满，1 盒子只有边框   0透显,1实显
    }
...
void player()
{
  ...
  if(selection == 1 && !inSideMenuSelection)
    ...
    if(playing)
    {
      u8g2.drawGlyph(midOriginX+7.5,midOriginY+7.5,68);    // 绘制特殊文本图标(需联合特殊字体)
    }
    else
    {
